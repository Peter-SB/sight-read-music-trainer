    ...
    <div className="page">
      ...
      <div className="scales-controls">
        <label className="field">
          ...
          <select value={keyMode} onChange={(e) => onKeyModeChange(e.target.value as KeyMode)}>
            <option value="concert">Concert pitch</option>
            <option value="written">{INSTRUMENTS[instrument].label} (written)</option>
          </select>
        </label>

        <label className="field">
          <span>Root {keyMode === 'written' ? '(as written)' : '(concert)'}</span>
        ...
        <label className="field">
          <span>Scale</span>
          <select value={scaleId} onChange={(e) => onScaleIdChange(e.target.value as ScaleId)}>
            {SCALE_LIST.map((s) => (
        ...
        <label className="field">
          <span>Range low</span>
          <select value={rangeLow} onChange={(e) => onRangeLowChange(e.target.value)}>
        ...
        <label className="field">
          ...
          <select value={rangeHigh} onChange={(e) => onRangeHighChange(e.target.value)}>
            {rangeOptions.map((note) => (
              <option key={note} value={note}>
                {note}
              </option>
        ...
        <div className="scales-controls__actions">
          <button
            ...
            onClick={() => onStartDrill(scaleId, concertRoot, rangeLow, rangeHigh, false)}
          >
            Start drill
          </button>
          <button
            ...
            className="mic-button"
            disabled={sequence.length === 0}
            onClick={() => onStartDrill(scaleId, concertRoot, rangeLow, rangeHigh, true)}
          >
            Drill random
          </button>
        </div>
      ...
      <div className="scale-preview">
        ...
        {sequence.length === 0 && <p>No notes fit this scale/range combination.</p>}
      </div>
    </div>
  )
}
